...
$(function () {
    $.ajax({
        ...
        dataType: "json",
        type: "get",
        async: false, //改成true，后面的curl/postman反而加载不了
        success: function (data) {
            //layui init 解析 json，模块化使用layer
    ...
    $("#postman").click(function () {
        var url = "/view/postman";
        $("#path-body").load(url);
    });

    $("#codegenerate").click(function () {
        var url = "/view/codegenerate";
        $("#path-body").load(url);
    });
...
//测试按钮，获取数据，operationId="postUserUsingPOST"
function getData(operationId) {
    ...
    if ("form" == parameterType) {
        ...
        try {
            ...
            layer.msg("请确保您已登陆且输入合法！ 错误信息：" + error, {icon: 5});
            return false;
        }
    }

    //发送请求，例如"/users"、"postUserUsingPOST"、"[object Object]"、"[object Object]"
...
function send(url, operationId, header, data) {
    ...
    if (hasFormData) {
        ...
        $.ajax({
            ...
            error:function(e){
                $("#json-response").html("");
                layer.msg("请确保您已登陆且输入合法！ 错误信息：" + JSON.stringify(e), {icon: 5});
            }
        });
        return;
    }

    //querystring ,将参数加在url后面，例如url="/users?id=1&name=hu&age=12&accessToken=lj"
...
//     //header("Access-Control-Allow-Origin: *"); 跨域处理
//     $("#swagger").click(function () {
//         var url = "localhost:8888:swagger-ui.html";
//         $("#path-body").load(url);
//         return false;
//     });
//
